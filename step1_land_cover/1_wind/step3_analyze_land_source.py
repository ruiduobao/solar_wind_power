# -*- coding: utf-8 -*-
"""
Land source analysis script (wind power version)
Author: 锐多宝 (ruiduobao)
Date: 2026-02-02
Description: 
    Read the summarized land cover data and the wind power Shapefile, and perform multi-dimensional statistical analysis and visualization.
    Includes global statistics, country statistics, latitude distribution trends and temporal change trends.
    Also exports the CSV data of all related charts.
"""
...
# ================= Configuration =================
# Input files (for wind power data)
ESRI_CSV = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\数据\土地覆盖数据\2018_2024的数据在2017年的土地覆盖上\WIND_ESRI_2017_LandCover_Summary.csv"
FROM_CSV = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\数据\土地覆盖数据\2018_2024的数据在2017年的土地覆盖上\WIND_FROM_2017_LandCover_Summary.csv"
SHP_FILE = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\数据\风机缓冲区\风机80米缓冲区_2017年之后的风机缓冲区.shp"

# Output directory
OUTPUT_DIR = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\制图\1.土地来源分析\风电"
...
def plot_global_composition(df, color_map, dataset_name, value_col='area_sqm'):
    ...
    plt.title(f"Global land cover composition of wind farm occupation ({dataset_name})", fontsize=16)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"Wind_Global_Composition_{dataset_name}.png"), dpi=300)
    plt.close()

    # Save CSV
    csv_path = os.path.join(OUTPUT_DIR, f"Wind_Global_Composition_{dataset_name}.csv")
...
def plot_country_analysis(df, color_map, dataset_name, top_n=10):
    ...
    plt.title(f"Land occupation share of the top {top_n} wind power countries ({dataset_name})", fontsize=16)
    plt.xlabel("Country", fontsize=12)
    plt.ylabel("Share (%)", fontsize=12)
    plt.legend(title="Land cover type", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"Wind_Country_Analysis_{dataset_name}.png"), dpi=300)
    plt.close()

    # Save CSV (save percentage data)
    csv_path = os.path.join(OUTPUT_DIR, f"Wind_Country_Analysis_{dataset_name}.csv")
...
def plot_latitude_distribution(df, color_map, dataset_name):
    ...
    plt.title(f"Latitude distribution of wind farm land cover types ({dataset_name})", fontsize=16)
    ...
    plt.legend(title="Land cover type", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"Wind_Latitude_Distribution_{dataset_name}.png"), dpi=300)
    plt.close()

    # Save CSV (save the original area data)
    csv_path = os.path.join(OUTPUT_DIR, f"Wind_Latitude_Distribution_{dataset_name}.csv")
...
def plot_yearly_trend(df, color_map, dataset_name):
    ...
    plt.title(f"Land source changes of wind farms by construction year ({dataset_name})", fontsize=16)
    plt.xlabel("Construction year", fontsize=12)
    plt.ylabel("Share (%)", fontsize=12)
    plt.legend(title="Land cover type", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"Wind_Yearly_Trend_{dataset_name}.png"), dpi=300)
    plt.close()

    # Save CSV (save percentage data)
    csv_path = os.path.join(OUTPUT_DIR, f"Wind_Yearly_Trend_{dataset_name}.csv")
    pivot_pct.to_csv(csv_path)
