...
// --- FROM-GLC10 (2017) ---
// Fix: removed .filterBounds(solar_fc.geometry())
...
var computeStats = function(image, feature) {
  ...
  var stats = stats_img.reduceRegion({
    ...
    scale: 10,             
    maxPixels: 1e9,
    tileScale: 4, // suggestion: slightly increase tileScale to prevent memory overflow for a single large solar farm
    bestEffort: true
  });
...
for (var i = 0; i < num_batches; i++) {
  ...
  Export.table.toDrive({
    ...
    description: 'SOLAR_ESRI_2017_Stats_For2018_2024_Batch_' + (i + 1),
    folder: 'GEE_solar_Analysis_2018_2024',
    fileFormat: 'CSV',
  ...
  Export.table.toDrive({
    ...
    description: 'SOLAR_FROM_2017_Stats_For2018_2024_Batch_' + (i + 1),
    folder: 'GEE_solar_Analysis_2018_2024',
    fileFormat: 'CSV',
    selectors: ['fid', 'stats_json'] 
  });
