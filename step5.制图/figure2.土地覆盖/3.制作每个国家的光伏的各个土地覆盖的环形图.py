# -*- coding: utf-8 -*-
"""
Create donut charts of land cover composition for each country
Author: 锐多宝
Date: 2026-02-02
Description:
    Draw a donut chart of land cover shares for each country from the given CSV data.
    Requirements:
    1. The country name is shown in the center.
    2. The shares of the top three land cover classes are shown in the ring.
    3. Font is Times New Roman.
    4. Custom colors.
"""
...
# 25: "Country_Analysis_FROM-GLC.csv" (assume this file contains raw area data rather than percentages; if it is percentages, accurate sorting is impossible)
# The previous script generated "Country_Analysis_FROM-GLC.csv" containing percentages.
# To sort by total area, we need the raw area data.
# Assume INPUT_FILE contains percentage data (inferred from the previous step),
# but the user now wants sorting by total area.
# Let us first check the content of INPUT_FILE to see whether it is percentages or areas.
# If it is percentages, we cannot know the total area; we can only assume the user wants the file order (usually already sorted) or we cannot re-sort.
# But the CSV generated by plot_country_analysis in the previous step is pivot_pct (percentages).
# To sort by total area, we need the area summary generated in plot_country_analysis, or the user-provided CSV is already the Top N sorted by area.

# Assume the row order in the CSV is already sorted by total area (plot_country_analysis usually sorts by total area and takes the Top N).
# So we only need to add the index while iterating.

# Input file path
INPUT_FILE = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\制图\figure2_全球光伏风电土地利用\子图\Solar_Country_Analysis_FROM-GLC.csv"

# Output directory
OUTPUT_DIR = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\制图\figure2_全球光伏风电土地利用\子图\光伏各个国家环形图_FROM-GLC"
...
# The user provided the data content; to make sure the script can run, we create the file first if it is missing
if not os.path.exists(INPUT_FILE):
    print(f"Input file not found; creating sample data at: {INPUT_FILE}")
...
def draw_donut_chart(country_name, data_series, rank):
    ...
    # Draw the donut chart (pie chart with a hole)
    # wedgeprops width controls the ring width
    # Increase the width to shrink the hole (0.3 -> 0.5)
    ring_width = 0.5
    wedges, texts = ax.pie(
        data, 
        startangle=90, 
        ...
        counterclock=False # Clockwise ordering
    )
    
    # 3. Add text in the ring (shares of the top 3)
    # Compute the center angle of each sector to place the text
    ...
    for i, (p, label) in enumerate(zip(wedges, data.index)):
        ...
        if label in top3_indices:
            ...
            # Radial position: in the middle of the ring (1 - width/2)
            ...
            # In matplotlib, 0 degrees is at the 3 o'clock position and increases counterclockwise
            # The ang here is already the standard angle
            
            # Normalize the angle to 0-360
            ang_norm = ang % 360
            
            # The tangential direction is usually ang - 90
            # To keep the text readable (not upside down), adjust according to the angle
            ...
            # For simplicity, use white with a stroke, or judge by color
            # Here we keep it simple: use white directly with a slightly larger font size
            ax.text(
                x * r, y * r, 
                f"{val:.1f}", 
                ha='center', va='center', 
    ...
    # Save - add the rank to the file name
...
def main():
    ...
    # Assume the input file is already sorted by total area (typical of previous analysis scripts)
    # If not sorted, we cannot sort here either, because the input is percentage data with the total area information lost.
    # We directly use the row number as the rank.
    ...
    for index, row in df.iterrows():
        ...
        # Extract the data columns (exclude the COUNTRY column)
