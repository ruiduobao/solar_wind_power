...
// =========================================================================
// Task: compute the land cover area and pixel count of ESRI and FROM-GLC10 (2017) within the wind farm boundaries
// =========================================================================
...
// Note: just mosaic directly; reduceRegion will request the data on demand, so do not manually merge large geometries for filtering
...
var computeStats = function(image, feature) {
  ...
  var stats = stats_img.reduceRegion({
    ...
    scale: 10,             
    maxPixels: 1e9,
    tileScale: 4, // Suggestion: slightly increase tileScale to prevent memory overflow for single large wind farm areas
    bestEffort: true
  });
...
// 3. Batched export logic

// Suggestion: the polygon computation is heavy; reduce the Batch Size to avoid task timeout or memory overflow in a single run
var BATCH_SIZE = 1000; 
