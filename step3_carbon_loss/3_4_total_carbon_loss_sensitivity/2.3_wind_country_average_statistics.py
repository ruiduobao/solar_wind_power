...
"""
2.3 统计风机的国家平均值.py
Function: compute the LULC carbon loss (soil + biomass) and manufacturing carbon emissions of wind
by country under three scenarios, and calculate the percentages.
Input: Wind_Total_Carbon_Debt_Result.csv under each scenario
Output: Wind_Country_Stats.csv under each scenario
Author: 锐多宝
Date: 2026-02-07
"""
...
# User path: Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\制图\4.4.2风机碳债务合并损失\
...
def process_scenario(scenario_name):
    ...
    input_file = os.path.join(input_dir, "Wind_Total_Carbon_Debt_Result.csv")
    ...
    try:
        ...
        # Output file path
        output_file = os.path.join(input_dir, "Wind_Country_Stats.csv")
        ...
        if os.path.exists(WORLD_SHP):
            try:
                ...
                maps_to_plot = [
                    ('LULC_Pct', "LULC Carbon Debt Percentage (%)", "Wind_LULC_Pct_Map.png"),
                    ('Soil_Pct', "Soil Carbon Debt Percentage (%)", "Wind_Soil_Pct_Map.png"),
                    ('Bio_Pct', "Biomass Carbon Debt Percentage (%)", "Wind_Bio_Pct_Map.png"),
                    ('Loss_Soil_tCO2', "Soil Carbon Debt (tCO2)", "Wind_Soil_Debt_Map.png"),
                    ('Loss_Bio_tCO2', "Biomass Carbon Debt (tCO2)", "Wind_Bio_Debt_Map.png")
                ]
                ...
                for col, legend_label, filename in maps_to_plot:
                    try:
                        ...
                        ax.set_title(f"Wind {col} by Country ({scenario_name})", fontsize=16)
                        ax.set_axis_off()
...
def main():
    print("Starting to aggregate wind carbon debt by country for each scenario...")
