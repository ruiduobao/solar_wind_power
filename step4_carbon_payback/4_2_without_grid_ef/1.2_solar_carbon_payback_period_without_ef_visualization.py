# -*- coding: utf-8 -*-
"""
Solar Carbon Payback Time (CPT) Visualization - Without Grid Emission Factors
Author: 锐多宝 (Trae AI)
Date: 2026-01-28

Functions:
1. Read the solar CPT result (Solar_CPT_No_Grid_EF.csv)
...
INPUT_CSV = os.path.join(OUTPUT_DIR, "Solar_CPT_No_Grid_EF.csv")
LOG_PATH = os.path.join(OUTPUT_DIR, "Solar_CPT_No_Grid_EF_Viz_Log.txt")
POWER_CSV = r"Z:\Mywork\论文\全球绿色能源生态评估_2025.12.24\制图\3.1光伏发电潜力计算\Solar_Power_Potential.csv" # used to obtain longitude/latitude
...
def export_plot_data(df, filename):
    ...
    log_message(f"Plot data exported: {csv_path}")
...
def plot_cpt_distribution(df):
    ...
    sns.histplot(plot_df['CPT_Years_NoEF'], bins=50, kde=True, color='#2ca02c')
    
    plt.title("Distribution of Solar CPT (Global Avg Grid EF)", fontsize=16, fontweight='bold')
...
def plot_cpt_vs_pvout(df):
    """CPT vs PVOUT scatter plot"""
    log_message("Plotting 2. CPT vs PVOUT scatter plot...")
    
    if 'pvout_daily_kwh_kwp' not in df.columns:
        log_message("Warning: PVOUT data missing, skipping plot.")
        return

    plot_df = df[df['CPT_Years_NoEF'] <= 20].copy()
    
    plt.figure(figsize=(10, 6))
    plt.hexbin(plot_df['pvout_daily_kwh_kwp'], plot_df['CPT_Years_NoEF'], gridsize=50, cmap='Greens', mincnt=1)
    cb = plt.colorbar(label='Count of Sites')
    
    plt.title("Solar CPT vs. Resource Potential (PVOUT) - No Grid EF Bias", fontsize=16, fontweight='bold')
    ...
    export_plot_data(plot_df[['pvout_daily_kwh_kwp', 'CPT_Years_NoEF']], filename)
...
def plot_country_ranking(df):
    ...
    plt.title("Top 20 Countries with Shortest CPT (Global Avg Grid EF)", fontsize=16, fontweight='bold')
...
def plot_cpt_map(df):
    ...
    if not os.path.exists(POWER_CSV):
        log_message("Warning: Power CSV not found, cannot obtain longitude/latitude, skipping map.")
        return
    ...
    plt.title("Global Distribution of Solar CPT (Global Avg Grid EF)", fontsize=16, fontweight='bold')
    plt.axis('off')
...
def main():
    ...
    log_message(">>> Start plotting solar CPT charts (without emission factors) ...")
    ...
    plot_cpt_vs_pvout(df)
    plot_country_ranking(df)
    plot_cpt_map(df)
    
    log_message("All charts have been plotted.")
